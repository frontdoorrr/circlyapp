...
This file provides guidance to AI coding agents (Claude Code, Codex, etc.) when working with code in this repository. Claude Code reads this via `@AGENTS.md` import in CLAUDE.md.
...
### Docker Compose 로컬 구동

Supabase 프로젝트가 paused 되었거나 외부 의존성을 피해서 iOS/Expo 기능 테스트를 해야 할 때는 `backend/docker-compose.yml`을 사용합니다.

```bash
# repo root에서 실행
docker compose -f backend/docker-compose.yml up -d db redis api

# 최초 구동 또는 DB 초기화 후
docker compose -f backend/docker-compose.yml exec api uv run alembic upgrade head
docker compose -f backend/docker-compose.yml exec api uv run python scripts/seed_templates.py
```

- `api`: 호스트 `8002` → 컨테이너 `8000`
- `db`: 호스트 `5433` → 컨테이너 `5432`
- `redis`: 호스트 `6379`
- `worker`: 현재 기본 구동 제외. `app.tasks.celery_app`가 준비되지 않았으므로 `worker` profile을 명시하지 마세요.
- compose 환경에서는 `DEV_AUTH_ENABLED=true`로 로컬 개발용 auth가 활성화됩니다.
...
기본 인증 구조는 Supabase Auth입니다. 단, Supabase가 paused 되었거나 로컬 Docker Compose 기반 테스트가 필요할 때만 development 전용 mock auth를 사용합니다.
...
`dev-login`은 이메일로 기존 로컬 사용자를 재사용하므로 같은 이메일로 로그인하면 Circle과 투표 데이터가 유지됩니다. 현재 development mock auth는 비밀번호를 저장하거나 검증하지 않으며, 위 비밀번호는 프론트 로그인 폼을 통과하기 위한 공용 테스트 값입니다. `docker compose down -v`로 PostgreSQL 볼륨을 삭제하면 mock user와 관련 데이터도 삭제됩니다.

- 백엔드 development 전용 엔드포인트: `POST /api/v1/auth/dev-login`
- 활성화 조건: `DEV_AUTH_ENABLED=true`이고 `APP_ENV=development`
- 발급 토큰 형식: `dev:<user_uuid>`
- 보호 API는 development에서만 `Authorization: Bearer dev:<user_uuid>`를 허용합니다.
- production/staging 코드나 배포 환경에서 `DEV_AUTH_ENABLED=true`를 사용하지 마세요.

프론트에서 mock auth를 쓰려면 `frontend/.env`에 다음 값을 둡니다:

```bash
EXPO_PUBLIC_AUTH_MODE=mock
EXPO_PUBLIC_API_URL=http://<개발머신 LAN IP>:8002/api/v1
```

Supabase가 정상일 때는 `EXPO_PUBLIC_AUTH_MODE=supabase` 또는 미설정 상태를 사용합니다.
...
```bash
# .env (로컬 개발)
EXPO_PUBLIC_API_URL=http://localhost:8002/api/v1
EXPO_PUBLIC_AUTH_MODE=supabase

# iOS/Expo Go에서 로컬 Docker Compose 백엔드 + mock auth 사용 시
EXPO_PUBLIC_API_URL=http://<개발머신 LAN IP>:8002/api/v1
EXPO_PUBLIC_AUTH_MODE=mock

# Supabase Auth 사용 시 필요
...
3. **API 응답 검증**:
   - Backend 서버 실행: `cd backend && make dev` 또는 `docker compose -f backend/docker-compose.yml up -d db redis api`
