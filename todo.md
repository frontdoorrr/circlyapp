...
- [ ] `app/(main)/(create)/questions/[category].tsx` - 질문 카드 스택 화면
  - [ ] 헤더: 뒤로가기 + 카테고리명 + 진행 표시 (3/8)
  - [ ] 카드 스택 컴포넌트 (최대 3장 표시)
    - [ ] 현재 카드: 343×480px, shadow-2xl
    - [ ] 뒷카드 1: 8px 아래, 8px 작게, opacity 0.7
    - [ ] 뒷카드 2: 16px 아래, 16px 작게, opacity 0.4
  - [ ] 카드 내부 레이아웃
    - [ ] 이모지 80px (중앙 정렬)
    - [ ] 질문 텍스트 (text-2xl, bold)
    - [ ] [선택하기] 버튼 (하단 고정)
- [ ] **스와이프 제스처** (React Native Gesture Handler + Reanimated)
  - [ ] 좌측 스와이프 (←): 다음 질문 (카드 왼쪽으로 사라짐)
    - [ ] 임계값: 100px
  ...
  - [ ] 위로 스와이프 (↑): 질문 선택 + 다음 단계로
    - [ ] 임계값: 80px
    - [ ] Scale + fade-out 애니메이션
    - [ ] Haptic: impact-medium
- [ ] **5개 액션 버튼**
  - [ ] ⬅️ 이전 (회색, 48px)
  - [ ] ❌ 건너뛰기 (빨강, 56px)
  - [ ] 💖 선택 (분홍, 64px, 메인) - Gradient primary
  - [ ] ✅ 선택 (파랑, 56px)
  - [ ] ➡️ 다음 (회색, 48px)
- [ ] 힌트 텍스트: "← 스와이프하여 넘기기" (3초 후 fade-out)
- [ ] **테스트**: 스와이프 제스처 및 버튼 동작 확인
- [ ] **커밋**: `feat(frontend): implement swipe card question selection`

#### 11.10.3 투표 설정 화면 (P0)
- [ ] `app/(main)/(create)/settings.tsx` - 투표 설정 화면
  - [ ] 선택한 질문 표시 (읽기 전용)
  - [ ] ⏰ 투표 기간 선택 칩 (1H/3H/6H/24H)
    - [ ] 비선택: white 배경, gray-200 테두리
    - [ ] 선택: primary-50 배경, primary-500 테두리
    - [ ] Haptic: selection
  - [ ] 🎯 참여 대상 라디오 옵션
    - [ ] ◉ Circle 전체 (15명)
    - [ ] ◯ 일부만 선택하기
  - [ ] 📢 알림 설정 라디오 옵션
    - [ ] ◉ 즉시 알림 보내기
    - [ ] ◯ 예약 발송
  - [ ] [미리보기] 버튼 (하단 고정, gradient-primary)
- [ ] **테스트**: 설정 선택 및 유효성 검사 확인
- [ ] **커밋**: `feat(frontend): implement poll settings screen`

#### 11.10.4 미리보기 화면 (P0)
- [ ] `app/(main)/(create)/preview.tsx` - 미리보기 화면
  - [ ] 안내 텍스트: "투표가 이렇게 보여요"
  - [ ] 프리뷰 컨테이너 (gray-50 배경)
    - [ ] 실제 투표 화면 70% 축소판
    - [ ] 2.3 투표 질문 화면 재사용
  ...
  - [ ] 액션 버튼 (2개, 좌우 배치)
    - [ ] [수정하기] - Secondary 버튼
    - [ ] [투표 시작] - Primary 버튼 (gradient-primary)
  - [ ] Haptic: impact-medium (투표 시작 시)
- [ ] **테스트**: 미리보기 표시 및 버튼 동작 확인
- [ ] **커밋**: `feat(frontend): implement poll preview screen`

#### 11.10.5 발행 완료 화면 (P0)
- [ ] `app/(main)/(create)/success.tsx` - 발행 완료 화면
  - [ ] Success 애니메이션
    - [ ] 🎉 이모지 120px
    - [ ] Scale bounce + rotate 애니메이션
      - [ ] From: scale 0.5, rotate -15deg
      - [ ] To: scale 1.2 → 1.0, rotate 15deg → 0deg
      - [ ] Duration: 600ms
      - [ ] Spring (stiffness 200, damping 15)
    - [ ] Confetti 파티클 효과 (react-native-confetti-cannon)
  - [ ] Success 메시지: "투표가 시작되었어요!"
  - [ ] 상세 정보: "15명의 친구에게 알림을 보냈어요"
  - [ ] 로딩 바 (자동 전환 표시)
    - [ ] 80px × 4px, gradient-primary
    - [ ] 0% → 100% in 3초
  - [ ] 자동 전환: 3초 후 Home 화면으로 fade-out
  - [ ] Haptic: notification-success
- [ ] **테스트**: 애니메이션 및 자동 전환 확인
- [ ] **커밋**: `feat(frontend): implement poll creation success screen`
