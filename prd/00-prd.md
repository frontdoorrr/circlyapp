...
- **카테고리별 분류**: 외모/분위기, 호감/크러시, 성격, 재능, 특별한 날 등으로 체계적 분류
- **서버 후보 생성**: 같은 Circle 멤버 중 투표자/생성자를 제외하고, 득표 편중을 줄이는 4지선다 후보를 서버가 생성
- **완전 익명 투표**: 누가 누구에게 투표했는지 절대 공개되지 않음

#### 💖 받은 하트 시스템
- **1급 화면화**: 사용자가 받은 칭찬/선택을 Profile 하위 메뉴가 아닌 핵심 탭에서 확인
- **무료 힌트**: 질문, 받은 수, Circle명, 시간대 등 안전한 맥락만 기본 제공
- **Orb Mode 연계**: 실명 공개가 아니라 단계형 안전 힌트로 궁금증을 해소
...
3. **투표하기**: Home에서 바로 투표 세션 시작 → 질문별 4명 후보 중 선택/건너뛰기/섞기
4. **라운드 완료**: 쿨다운, 친구 초대, 알림 허용 CTA를 통해 다음 참여 동기 제공
5. **받은 하트 확인**: 받은하트 탭에서 내가 선택받은 질문과 안전 힌트 확인
6. **공유하기**: 예쁜 결과 카드를 SNS에 공유하여 추억 보관

### 차별화 포인트
- **건전한 문화**: 칭찬과 긍정적 평가 위주로 사이버불링 방지
- **외모/호감 질문의 안전화**: 잘생김, 분위기, 고양이상, 설렘 같은 관심사는 핵심 카테고리로 다루되 비하/상처/부정 비교형 질문은 금지
- **기본 익명성 + Orb Mode**: 일반 사용자에겐 익명, Orb Mode는 실명 공개가 아닌 단계형 안전 힌트로 제공
...
| P0 | 연속 투표 세션 | Home에서 바로 시작하는 4지선다 질문 루프. Skip/Shuffle 제공, 후보 부족 시 투표 화면 대신 친구 초대 화면을 우선 표시 |
| P0 | 받은 하트 탭 | 내가 선택받은 칭찬을 별도 탭에서 확인. 무료 힌트와 읽음 상태를 제공하고 Orb Mode CTA와 연결 |
...
2. **투표 참여**: Home 탭에서 진행 중 세션을 바로 시작 → 질문 확인 → 4명 후보 중 선택/Skip/Shuffle
3. **세션 완료**: 정해진 질문 수 완료 → 쿨다운 화면 → 친구 초대 또는 알림 허용 CTA
4. **받은 하트 확인**: 받은하트 탭에서 내가 선택받은 질문 확인 → 무료 힌트 확인 → 필요 시 Orb Mode 힌트 CTA
5. **결과 공유**: 투표 마감 시 예쁜 결과 카드 자동 생성 → 인스타그램/카카오톡 등 SNS에 원터치 공유
6. **지속 참여**: 푸시 알림과 받은 하트 배지를 통해 새로운 투표/칭찬 알림 받기 → 반복적 참여로 Circle 내 활발한 소통 유지

## 8. UI/UX 원칙
• 4탭 구조: Home(투표 세션) / 받은하트(Inbox) / Circle / Profile
• Home은 대시보드가 아니라 즉시 투표를 시작하는 Play Lobby 역할을 한다.
• 일반 사용자는 관리자/템플릿 기반 질문에 참여한다. 투표 생성 기능은 MVP에서 핵심 탭이 아니라 관리자 또는 Circle 상세의 제한된 진입점으로 둔다.
• 받은하트는 보상 루프의 중심 화면이며, Orb Mode CTA는 이 맥락에서 안전 힌트로 연결한다.
...
| 익명성 악용 | 기본 익명성을 유지하고, Orb Mode도 실명 공개가 아닌 단계형 안전 힌트로 제한. Circle 단위 제재와 신고 흐름으로 건전성 유지 |
...
- Flame(불꽃)처럼 받은 반응을 모아보는 보상 루프
- 시간제 라운드(1시간마다 갱신)로 FOMO 극대화

**Circly 적용 제안**:
- Home을 Play Lobby로 구성하고 질문당 4지선다 투표 + Skip/Shuffle 버튼 제공
- Inbox/받은하트 탭을 핵심 보상 화면으로 승격
- 후보 부족 시 빈 투표 UI 대신 초대 우선 화면으로 전환
- 외모/호감/크러시 질문은 긍정형으로 적극 활용하되 부정형/비하형 질문 차단
- 라운드 완료 후 쿨다운, 초대, 알림 CTA로 다음 참여 루프 형성

**Gas vs TBH 비교**:
| 항목 | Gas | TBH |
|------|-----|-----|
| 보상 | Flames | Gems |
| 수익화 | Orb Mode 구독 | 인앱 동기 부여 |
| 서비스 | 2023년 Discord 인수 | 2018년 종료 |

**Circly Orb Mode**:
- 유료 구독 기능 (RevenueCat 연동)
- 핵심 가치: 받은 하트 맥락에서 단계형 안전 힌트 제공
- 일반 사용자에겐 투표자 익명 유지
- 미성년자 대상 서비스이므로 직접적인 실명 공개, 성별 색상 힌트, 괴롭힘으로 이어질 수 있는 특정성 높은 노출은 제한
