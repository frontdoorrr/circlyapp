...
Liquid Glass는 콘텐츠 카드 전체가 아니라 내비게이션과 주요 조작부를 위한 별도 기능 레이어로 사용합니다.

- **Native**: iOS 26 이상은 `expo-glass-effect`의 `GlassView` 사용
- **Legacy iOS**: `expo-blur`의 system material 사용
- **Android**: 성능을 위해 반투명 surface와 border로 fallback
- **접근성**: `Reduce Transparency` 활성화 시 불투명 surface 사용
- **적용 우선순위**: 하단 탭바 → 상단 헤더 → 선택 컨트롤/주요 CTA → 바텀시트
- **메인 탭바**: 화면 하단과 좌우에서 분리된 캡슐형 glass surface에 4개 탭을 배치하고 선택 아이콘만 Primary Violet 원형으로 강조
- **퀵 액션**: 캡슐 오른쪽의 독립 원형 glass control은 투표 세션 시작에 사용하며 쿨다운 중에는 hourglass 상태로 비활성화
- **콘텐츠 여백**: 플로팅 탭바가 스크롤 콘텐츠나 화면 FAB를 가리지 않도록 최소 120px의 하단 안전 여백 확보
- **메인 탭 배경**: Home, 하트, Circle, Profile 모두 장식용 원형 오브 없이 동일한 저채도 Violet/Pink 3단 그라데이션을 사용
- **콘텐츠 계층**: 투표·Circle 카드는 읽기 쉬운 standard material 또는 반투명 card를 사용하고 Liquid Glass를 반복 적용하지 않음
- **색상**: Primary Violet은 선택 상태나 옅은 tint에만 사용하며, 글라스 위 본문 텍스트는 WCAG AA 대비를 유지
- **모션**: GlassView 또는 상위 뷰의 `opacity` 애니메이션 대신 scale, position, native glass transition 사용

공용 구현은 `frontend/src/components/primitives/GlassSurface.tsx`를 기준으로 하며 화면에서 `GlassView`와 `BlurView`를 직접 분기하지 않습니다.
