...
투표 생성자는 템플릿이 허용하는 범위 안에서 후보 풀을 좁힐 수 있다.

- 성별 기반 후보 제한은 사용자가 Profile에서 선택 입력한 값만 사용한다.
- 성별 정보는 기본 비공개이며 투표 생성자에게 개별 값이 노출되지 않는다.
- 필터 옵션은 `전체`, `남자`, `여자`, `논바이너리`, `선택 안 한 사람 포함/제외`를 기본으로 한다.
- 필터 적용 후 후보가 4명 미만이면 생성 전 경고를 띄우고 제한 완화 또는 Circle 초대를 유도한다.
