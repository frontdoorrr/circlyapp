...
## 기능 개요
Circle 생성자가 영구 초대 링크와 24시간 유효한 6자리 코드를 생성하여 친구들을 Circle에 초대하고, 앱 미설치자도 쉽게 참여할 수 있도록 하는 기능
...
- **영구 링크 유효기간**: Circle이 활성 상태인 동안 유지
- **6자리 코드 유효기간**: 발급/재발급 시점부터 24시간
- **사용 횟수**: 설정된 인원 제한까지
...
초대 코드는 24시간 후 만료돼요. 링크는 계속 사용할 수 있어요! 📱
```
...
#### 6.1 딥링크 스키마
- 앱 스킴(iOS/Android 공통): `circly://join/{invite_link_id}`
- Universal/App Link: `https://circly.app/join/{invite_link_id}`
- 6자리 코드는 앱의 코드 입력 화면에서만 사용하며 영구 링크 파라미터로 변환하지 않는다.

#### 6.2 딥링크 처리 로직
1. 앱 실행 시 딥링크 파라미터 확인
2. 영구 `invite_link_id`로 Circle 상태와 정원을 검증
...
#### 8.1 초대 링크/코드 오류
- **비활성/잘못된 링크**: "사용할 수 없는 초대 링크예요."
- **만료된 코드**: "초대 코드가 만료되었어요. 새로운 코드를 요청해주세요."
